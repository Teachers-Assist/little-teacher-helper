...
describe('detectTakeOver（US9 接手提示判定）', () => {
  ...
  // ── 回歸：測試回饋回報的三段輪替（8 登 → 11 接手 → 8 再接手 → 8 重進） ──
  // 舊的「多數決 / 先濾掉自己」判定在這裡會答 11（已交棒的舊接手者），
  // 讓 8 號重進時被問「要接手 11 號嗎?」。以最新一筆為準後，8 就是目前接手者。
  describe('三段輪替 8 → 11 → 8 的回歸情境', () => {
    const chain = {
      s1: rec(8, '2026-08-10T10:00:00.000Z'),
      s2: rec(8, '2026-08-10T10:01:00.000Z'),
      s3: rec(11, '2026-08-10T10:30:00.000Z'),
      s4: rec(8, '2026-08-10T11:00:00.000Z'),
    };

    it('8 號再接手後退出重進 → 不提示（自己就是目前接手者）', () => {
      expect(detectTakeOver(chain, 8)).toBeNull();
    });

    it('同一份資料下 11 號進來 → 提示接手 8（而非仍指向自己）', () => {
      expect(detectTakeOver(chain, 11)).toBe(8);
    });

    it('第三人進來 → 提示最新的 8，而非登得最多的 8 以外的任何人', () => {
      expect(detectTakeOver(chain, 20)).toBe(8);
    });
  });
});
