...
└─────────────┘       └─────────────┘       └─────────────┘
         ...
         Task ─1:N─┐                    ┌─1:N─ Student
                   └─→ ┌───────────────┐ ←┘
                       │ RecordHandler │  （004：某一格的順序處理者名單）
                       └───────────────┘
      key 為 (taskId, studentId)，與 Record 平行而非其子表——
      該格的登記被刪除時，經手鏈仍保留（見 §6）
```
...
**某一格 (Task, Student) 的順序處理者名單**：每一次處理（建立、修改或刪除）追加一筆，依時間排序，構成完整經手鏈。目的是「監視器」證據 —— 老師起疑時可查「這一格被哪些座號、依什麼順序、在什麼時間處理過」（004 US4）。
...
> **2026-08-10 變更（migration `0002_record_handler_by_cell.sql`）**：key 由 `Record.id`
> 改為 `(taskId, studentId)`。原本掛在 Record 上並帶 `ON DELETE CASCADE`，一旦該格的登記
> 被刪除（清空成績、取消勾選）整條鏈跟著消失；而「把 80 改成 90」在實務上常是先清空再
> 重打，於是鏈被反覆抹掉，老師看不出這一格曾被多人動過。
>
> **經手歷史屬於那一格，不屬於「目前存在的那筆登記」。** 因此本表允許出現「該格已無
> Record、但鏈還在」的列 —— 那正是要保留的證據，不是待清的孤兒。生命週期改由
> Task / Student 連坐。
...
- 刪除一筆登記 MUST 追加 `action='DELETE'`，MUST NOT 清空名單 —— 清掉別人登的資料正是老師最需要看見的經手
