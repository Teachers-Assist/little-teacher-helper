...
- [ ] T101 [P] 在 `prisma/schema.prisma` 為 Task model 新增 `isArchived Boolean @default(false)` 欄位
- [ ] T102 在 root 執行 Prisma migration：`pnpm prisma migrate dev --name add-task-archived`
- [ ] T103 [P] 在 `src/types/index.ts` 補上 `Task.isArchived` 型別
- [ ] T104 [P] 安裝 Excel 解析套件：`pnpm add xlsx`（評估 SheetJS 與 exceljs，擇一）
- [ ] T105 [P] 在 `src/lib/excelTemplate.ts` 建立範本檔產生器（座號、姓名兩欄 + 一列範例）
- [ ] T106 [P] 將靜態範本 `public/templates/students-template.xlsx` 預先產生並 commit
...
- [ ] T107 [US1] 在 `src/lib/excelParser.ts` 實作 Excel 解析：欄位驗證、檔案內衝突偵測、回傳結構化錯誤
- [ ] T108 [US1] 在 `src/app/api/rooms/[id]/students/import/route.ts` 新增 POST endpoint：接收檔案、呼叫 parser、檢查與既有資料衝突、整批寫入
- [ ] T109 [US1] 在 `src/app/api/rooms/[id]/students/template/route.ts` 新增 GET endpoint：回傳範本檔（或直接服務靜態檔）

### UI

- [ ] T110 [P] [US1] 在 `src/components/StudentImport.tsx` 新增匯入元件：下載範本按鈕、上傳區、衝突列表顯示
- [ ] T111 [US1] 將 `StudentImport` 整合進 `src/app/teacher/rooms/new/page.tsx` 學生名單區塊
- [ ] T112 [US1] 將 `StudentImport` 整合進 `src/app/teacher/rooms/[id]/page.tsx` 學生 tab 側欄（與「新增單筆」並列）

### i18n

- [ ] T113 [P] [US1] 在 `src/i18n/messages.ts`（或對應檔）新增匯入相關訊息：成功、衝突類型描述、檔案錯誤
...
- [ ] T114 [US2] 在 `src/app/api/rooms/[id]/students/route.ts` 補上 GET query: `includeRemoved=true` 支援
- [ ] T115 [US2] 在 `src/app/api/rooms/[id]/students/[studentId]/route.ts` 新增 PATCH（編輯）、DELETE（soft delete，設 `isRemoved=true`）、POST `/restore`（還原）

### UI 元件

- [ ] T116 [P] [US2] 新增 `src/components/StudentForm.tsx`：**內嵌表單**（非 modal），取代目前 `rooms/[id]/page.tsx` 內嵌的新增學生表單；支援「新增」與「編輯」兩種模式，欄位含座號、姓名；編輯模式顯示「正在編輯：[姓名]」與「取消編輯」按鈕。**與 `TaskForm` 共用相同的視覺結構與行為樣式**
- [ ] T117 [P] [US2] 修改 `src/components/StudentRoster.tsx`（US5 拆分後的元件）：每列加上編輯 / 移除 icon
- [ ] T118 [P] [US2] 新增 `src/components/RemovedStudentsDrawer.tsx`：已移除學生清單 + 還原按鈕

### UI 整合

- [ ] T119 [US2] 修改 `src/app/teacher/rooms/[id]/page.tsx` 學生 tab：
...
- [ ] T120 [US3] 修改 `src/app/teacher/rooms/[id]/page.tsx`：將「班級資訊」卡片從每個 tab 的側欄拉出，改放於 tab 列**之上**，所有 tab 共用一份
- [ ] T121 [US3] 同步移除各 tab 側欄內重複的班級資訊區塊（學生 tab 的「房間資訊」、任務 tab 的「房間資訊」、班級狀況 tab 的「房間資訊」）

### API

- [ ] T122 [US3] 在 `src/app/api/tasks/[roomId]/route.ts` 補上 GET query: `includeArchived=true`，預設過濾 `isArchived=false`
- [ ] T123 [US3] 在 `src/app/api/tasks/[roomId]/[taskId]/route.ts` 補上 PATCH `isArchived=true`（封存）與 POST `/restore`（還原）

### UI 元件

- [ ] T124 [P] [US3] 新增 `src/components/TaskForm.tsx`：**內嵌表單**（非 drawer），支援「新增」與「編輯」兩種模式，欄位含名稱、類型、指定座號、截止時間；編輯模式顯示「正在編輯：[任務名]」與「取消編輯」按鈕。
...
- [ ] T125 [P] [US3] 新增 `src/components/ArchivedTasksDrawer.tsx`：已封存任務清單側拉抽屜 + 還原按鈕

### UI 整合

- [ ] T126 [US3] 修改 `src/app/teacher/rooms/[id]/page.tsx` 任務 tab：
...
- [ ] T126a [US3] 在 `src/lib/task.ts` 新增 `getTaskDisplayState(task): { badge, actions }` 純函式：依 `(status, dueDate, now)` 派生徽章與按鈕清單；老師端列項與 `TaskForm` 共用此邏輯
- [ ] T126b [P] [US3] 整合對話框文案到 i18n（`messages.teacher.taskStatusExtendDue` / `taskStatusReopen` / `taskStatusClose` / `taskBadgeDueExpired` 等）
...
- [ ] T127 [US4] 在 `src/app/api/rooms/[id]/monitoring/route.ts` 新增 GET endpoint：回傳簡易統計（總數、進行中、有異常、已封存）+ 警告清單（每筆含 taskId、異常類型、發生時間 / 閾值資訊）
- [ ] T128 [US4] 在 `src/lib/anomalyDetection.ts` 實作異常判斷邏輯，閾值預設值寫入並由 plan.md 紀錄：指定座號 24h 無登記、所有人未登記且距截止 < 6h、裝置長時間未同步（具體閾值由 plan.md 定義）

### UI

- [ ] T129 [P] [US4] 新增 `src/components/MonitoringStats.tsx`：簡易統計卡片區塊
- [ ] T130 [P] [US4] 新增 `src/components/MonitoringAlerts.tsx`：警告清單區塊（空白狀態 + 警告卡片），點警告 `router.push('/teacher/tasks/{roomId}/{taskId}')`
- [ ] T131 [US4] 修改 `src/app/teacher/rooms/[id]/page.tsx` 班級狀況 tab：
...
- [ ] T132 [US5] 從 `src/components/StudentList.tsx` 複製出 `src/components/StudentRoster.tsx`：唯讀、無 checkbox、預留 icon 插槽（在 US2 填入）
- [ ] T133 [US5] 從 `src/components/StudentList.tsx` 複製出 `src/components/StudentChecklist.tsx`：保留現有勾選邏輯、submission state、aria
- [ ] T134 [US5] 修改 `src/app/teacher/rooms/[id]/page.tsx`：使用 `StudentRoster` 取代 `StudentList`
- [ ] T135 [US5] 修改 `src/app/helper/[roomId]/[taskId]/page.tsx`：使用 `StudentChecklist` 取代 `StudentList`
- [ ] T136 [US5] 刪除 `src/components/StudentList.tsx`（確認無其他引用後）
...
- [ ] T137 [US6] 刪除 `src/app/teacher/tasks/[roomId]/page.tsx`（不做 redirect handler）
- [ ] T138 [US6] 確認專案內無其他元件 import 或 link 至 `/teacher/tasks/[roomId]`（grep `tasks/${roomId}` / `tasks/[roomId]`，把連結改成 `rooms/[roomId]?tab=tasks`）

### 細節頁

- [ ] T139 [US6] 新增 / 改造 `src/app/teacher/tasks/[roomId]/[taskId]/page.tsx`：
...
- [ ] T140 [P] [US6] 從 `src/components/ReportView.tsx` 抽出「單任務結果視圖」可重用元件 `src/components/TaskResultView.tsx`，供細節頁使用
- [ ] T141 [US6] 修改 `src/app/teacher/rooms/[id]/page.tsx` 任務 tab 列項：點任務名稱（非 icon）使用 `<Link href="/teacher/tasks/{roomId}/{taskId}">`
...
- [ ] T146 [US7] 若 `src/components/ui/Toast.tsx`（或對應 hook）尚未存在，建立基礎 toast 元件 + Provider；提供 `useToast()` 介面
- [ ] T147 [US7] 若 `src/components/ui/Modal.tsx`（或 Dialog 元件）尚未存在，建立基礎 modal 元件，支援 ESC 關閉、點外側關閉、aria

### QRCode Modal

- [ ] T148 [US7] 新增 `src/components/QRCodeModal.tsx`：
  - 滿版黑色半透明背景 + 中央白卡
...
- [ ] T149 [US7] 修改 `src/app/teacher/rooms/[id]/page.tsx`：
...
- [ ] T150 [US7] 刪除 `src/app/teacher/rooms/[id]/qrcode/page.tsx`（網站未上線、不需 redirect）
- [ ] T151 [US7] 整理 i18n：
...
- [ ] T156 [US8] 新增 `src/app/api/teachers/[id]/dashboard/route.ts` GET endpoint，回傳結構：
  ```
  {
...
- [ ] T157 [US8] 改造 `src/app/teacher/page.tsx`：
  - 刪除既有班級卡片網格區塊
...
- [ ] T158 [P] [US8] 新增 `src/components/dashboard/DashboardStats.tsx`：橫向三格卡片，顯示班級數、進行中任務數、異常數
- [ ] T159 [P] [US8] 新增 `src/components/dashboard/ClassesView.tsx`（按班級檢視 tab 內容）：
  - 每班一張卡片，顯示：班級名（大字）+ 進行中任務數 + 異常數紅點（無則隱藏）+ 最近活動時間（相對時間：「3 小時前」、「今早」、「昨天」等）
  - 點卡片跳 `/teacher/rooms/{id}`
  - Grid 排版（sm:grid-cols-2、lg:grid-cols-3）
- [ ] T160 [P] [US8] 新增 `src/components/dashboard/TasksView.tsx`（按任務檢視 tab 內容）：
  - 頂部搜尋框（佔位文字「🔍 搜尋任務名稱」）
...
- [ ] T161 [US8] 修改 `src/components/layout/TeacherSidebar.tsx`：
  - **刪除** line 65 的「班級」偽按鈕
...
- [ ] T162 [P] [US8] 新增 `src/components/layout/TeacherSidebarClassList.tsx`（封裝展開清單邏輯，獨立可測）

### 響應式與返回連結

- [ ] T163 [US8] 全站掃描 `/teacher/*` 頁面，**移除所有 page-header 內的「返回儀表板」/「返回房間」/「返回某某」連結**（包含 `rooms/[id]/page.tsx`、`tasks/[roomId]/[taskId]/page.tsx`、`rooms/new/page.tsx` 等所有 page-header）
- [ ] T164 [US8] 修改 `src/app/teacher/layout.tsx`：
  - 桌機尺寸（≥ 768px）：側欄常駐
  - 小螢幕（< 768px）：側欄收合為抽屜模式
  - page-header 左側加 hamburger 按鈕（僅小螢幕顯示），點擊開側欄抽屜
- [ ] T165 [P] [US8] 新增 i18n keys：
...
- [ ] T166 在 `specs/ui-spec.md` 新增「互動模式判斷規則」段落（班級資訊上提、inline 表單模式切換、可逆動作一層確認、QRCode 採 modal 是「展示 / 分享」例外不是「編輯資料」、側欄為唯一導航中樞、小螢幕 hamburger 模式、dashboard 雙視角設計原則），對應 NFR-006 / NFR-007
...
- [ ] T168 在 `specs/001-little-teacher-helper/plan.md` 註記 002 feature 已擴充 Task entity 與路由結構（QRCode 頁面廢除、tasks/[roomId] 列表頁廢除、dashboard 改造）
- [ ] T169 執行 `spec-align` skill 檢查所有文件一致（含 i18n 對齊；spec-align skill 會由使用者擴充以涵蓋 `src/i18n/messages.ts`）
