...
/teacher/rooms/[id]
  ...
  ├─ 學生 tab    ：列表 + 側欄新增/編輯/移除入口
  ├─ 任務 tab    ：列表 + 側欄新增（inline，與學生 tab 互動模式一致）
  │              + 列項 icon：編輯（將該任務載入側欄表單）+ 封存
  │              + 點任務名稱 → 跳 /teacher/tasks/[roomId]/[taskId]
...
3. **Given** 老師在細節頁, **When** 老師查看登記列表, **Then** 每一筆顯示：學生姓名 / 座號、登記結果（繳交或成績值）、登記者座號、登記時間
4. **Given** 老師在細節頁, **When** 老師查看未登記學生, **Then** 系統顯示「未登記」區塊，列出尚未有 Record 的學生
5. **Given** 老師在細節頁, **When** 老師點「匯出為列印格式」或「複製為文字」, **Then** 行為與原 spec FR-009 / FR-010 一致
6. **Given** 老師在細節頁, **When** 老師需要修改任務設定（名稱、指定座號等）, **Then** 頁面提供「回任務 tab 編輯」連結，**不在當前頁就地編輯**
...
- **FR-036**: 系統 MUST 改造 `/teacher/tasks/[roomId]/[taskId]` 為單一任務的「結果頁」：顯示任務資訊、完整登記列表、未登記學生清單、匯出操作；**不在此頁編輯任務設定**
