...
### User Story 7 - 任務細節頁 + 多人經手展示（換座號登記同一任務）(Priority: P1)

（2026-07-31 擴充）為推廣本專案招牌特色「多人經手」（順序處理者留痕，源自 004 US4），示範補上**老師端任務細節頁**與**小老師端換座號**能力，讓試用者親手走完整迴圈：**小老師用座號 A 登記 → 換座號 B 再登記同一位同學 → 老師端任務細節頁看到該筆標「多人經手」、可展開經手順序**。

**Why this priority**: 「多人經手」是本專案「讓班級自治仍留痕可查」的核心信任機制，原示範完全沒展示（老師端只有任務清單、小老師端座號固定）。這條迴圈把「誰先誰後經手同一筆」演給試用者看，補齊需求 1（體現特色）最關鍵的一塊。

**Independent Test**: `/demo` 點任務 → 進細節頁見全班登記明細（已登記者附「登記者 N 號・時間」）；小老師端對某生以座號 1 登記、點「登記者：1 號」換成座號 2、再登記同一生 → 老師端細節頁該筆出現琥珀「多人經手」badge、可展開見「1 號→2 號」順序（兩個時間戳）。

**Acceptance Scenarios**:

1. **Given** `/demo` 任務清單, **When** 使用者點任一任務, **Then** MUST 進入該任務**細節頁**（老師端視角）：摘要（繳交型已交/未交/繳交率；成績型 N/總）+ 全班登記明細（已登記者顯示登記者座號與時間）
2. **Given** 小老師端登記畫面, **When** 呈現, **Then** MUST 提供**換座號**入口（點「登記者：座號」badge → 座號選擇），且 MUST 有引導提示邀請「換座號重登同一位同學看多人經手」
3. **Given** 同一 (任務, 學生) 被**≥2 個不同座號**先後登記, **When** 老師端細節頁渲染該筆, **Then** MUST 顯示「多人經手」標示，且可展開完整經手順序（各座號 + 時間）
4. **Given** 經手鏈維護, **When** 判定是否追加一手, **Then** MUST 由**真實純規則 `shouldAppendHandler`**（與正式寫入路徑 `recordWrite.ts` 同一函式）決定「與名單最後一筆座號不同才追加」，非 demo 寫死
5. **Given** 小老師端換座號後登記, **When** 該筆同步至老師端視窗, **Then** MUST 經 `BroadcastChannel` 一併帶最新經手鏈，老師端細節頁才反映得出「多人經手」（守 ISO-2/4：不碰 D1、不假設 sessionStorage 互通）

> **細節頁做法**：採**自寫平行元件 `DemoTaskDetail`**，比照本 feature 既有 `DemoQrModal` / `DemoSyncIndicator`——**不重用** `TaskResultView`（其 `fetch('/api/records')` 碰 D1、違反 ISO-2；經手鏈子元件 `HandlerTrail` 未匯出），僅沿用其視覺語言與既有 i18n（`teacher.taskDetail.*` / `report.*`）。細節頁採 in-page state（非新路由），避免老師端視窗的 `BroadcastChannel` 因換路由而斷線。
...
- **FR-154**（US7）: `/demo` 老師端任務清單 MUST 可點入該任務**細節頁**：摘要 + 全班登記明細（已登記者顯示登記者座號與時間）。細節頁 MUST 為**自寫平行元件**（不重用 `TaskResultView` 之 `/api/records` fetch 路徑），沿用既有 `teacher.taskDetail.*` / `report.*` 文案；MUST 採 in-page state 而非新路由（保住老師端 `BroadcastChannel` 不斷）
- **FR-155**（US7）: `/demo/helper` 小老師端 MUST 提供**換座號**入口（點「登記者：座號」badge → 重用 `SeatSelector` 選座號 → `setDemoSeat`），並 MUST 有引導提示邀請「換座號重登同一位同學看多人經手」
- **FR-156**（US7）: demo store MUST 維護與 records 平行的**經手鏈**（`handlers[taskId][studentId]`）；追加與否 MUST 由真實純規則 `shouldAppendHandler`（`src/lib/recordHandlerRule.ts`，與正式 `recordWrite.ts` 同源）決定；同一 (任務,學生) 經 ≥2 個不同座號時老師端細節頁 MUST 標「多人經手」並可展開順序；跨視窗同步 MUST 一併廣播經手鏈（`DemoSyncMessage.handlers`）
...
- **任務 B — 數學小考（GRADE，成績型）**：種子預設數位同學已有成績。體現：成績數值登記、只能輸入數字的資料型別、班級成績概況。**多人經手最佳示範場**：試用者以座號 A 輸入某生成績、換座號 B 修改同一生成績，即產生 ≥2 座號經手鏈（成績可重覆 upsert，不像繳交勾選再點會取消）。
- **經手鏈種子（US7）**：所有已登記種子記錄皆掛**單筆**經手鏈（座號 1）→ 初始不觸發「多人經手」，留給試用者親手換座號製造。
...
- **SC-045**（US7）: 小老師端以座號 A 登記某生、換座號 B 再登記同一生後，老師端任務細節頁該筆 MUST 標「多人經手」並可展開見兩座號順序；經手鏈追加由真實 `shouldAppendHandler` 決定（程式碼可追溯，非寫死）
